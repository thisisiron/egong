...
const VIEW_BTN = (active: boolean) =>
  active
    ...
    : 'px-3 py-1.5 rounded bg-white border border-slate-200 text-slate-600 text-sm hover:bg-gray-50 hover:border-gray-300'
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 text-sm">
          {view === 'month' && (
            <>
              <Link href={`/teacher?view=month&ym=${prevYm}`} className="px-2 py-1 rounded hover:bg-gray-50">‹</Link>
              <span className="font-semibold">{range.label}</span>
              <Link href={`/teacher?view=month&ym=${nextYm}`} className="px-2 py-1 rounded hover:bg-gray-50">›</Link>
            </>
          )}
          {view === 'year' && (
            <>
              <Link href={`/teacher?view=year&y=${year - 1}`} className="px-2 py-1 rounded hover:bg-gray-50">‹</Link>
              <span className="font-semibold">{range.label}</span>
              <Link href={`/teacher?view=year&y=${year + 1}`} className="px-2 py-1 rounded hover:bg-gray-50">›</Link>
            </>
          )}
