...
import { ChildAssignment } from '@/lib/students/components/ChildAssignment'
...
import type { StudentDetailView as StudentDetailViewData, StudentNote } from '@/lib/students/types'
import type { SessionUser } from '@/lib/auth'

export interface StudentDetailViewProps {
  view: StudentDetailViewData
  notes: StudentNote[]
  user: SessionUser
...
export async function StudentDetailView({
  ...
  const attendance = await Promise.all([
    getAttendanceRate(student.id, range.from, range.to),
    getAttendanceCounts(student.id, range.from, range.to),
    getStudentAttendanceWithDates(student.id, range.fromIso, range.toIso),
  ]).catch((e: unknown) => {
